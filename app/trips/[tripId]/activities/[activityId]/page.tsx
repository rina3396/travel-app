...
export default function ActivityDetailPage({ params }: { params: Promise<{ tripId: string; activityId: string }> }) {
  ...
  return (
    <section className="mx-auto w-full max-w-2xl space-y-6 p-4">
      ...
      {loading ? (
        <Card title="Activity Details" description="Edit activity information.">
          ...
          <div className="grid gap-4">
            <label className="grid gap-1 text-sm" htmlFor="title">
              ...
              <div className="relative">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" aria-hidden>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 5h18M3 12h18M3 19h18" />
                </svg>
                <input id="title" value={item.title} onChange={(e) => setItem({ ...(item as any), title: e.target.value })} placeholder="例: Museum Tour" aria-describedby="title-help" className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 pl-9 text-sm placeholder-gray-400 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-200/60" />
              </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <label className="grid gap-1 text-sm" htmlFor="start">
                ...
                <div className="relative">
                  ...
                  <input id="start" type="time" value={item.start_time ?? ''} onChange={(e) => setItem({ ...(item as any), start_time: e.target.value })} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 pl-9 text-sm outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-200/60" />
                </div>
              </label>
              <label className="grid gap-1 text-sm" htmlFor="end">
                ...
                <div className="relative">
                  ...
                  <input id="end" type="time" value={item.end_time ?? ''} onChange={(e) => setItem({ ...(item as any), end_time: e.target.value })} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 pl-9 text-sm outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-200/60" />
                </div>
              </label>
            </div>
            ...
            <label className="grid gap-1 text-sm" htmlFor="location">
              ...
              <div className="relative">
                ...
                <input id="location" value={item.location ?? ''} onChange={(e) => setItem({ ...(item as any), location: e.target.value })} placeholder="例: Downtown" aria-describedby="location-help" className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 pl-9 text-sm placeholder-gray-400 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-200/60" />
              </div>
            ...
            <label className="grid gap-1 text-sm" htmlFor="note">
              ...
              <div className="relative">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-gray-400" aria-hidden>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h10" />
                </svg>
                <textarea id="note" value={item.note ?? ''} onChange={(e) => setItem({ ...(item as any), note: e.target.value })} placeholder="補足情報やメモ" aria-describedby="note-help" className="min-h-[140px] w-full rounded-lg border border-gray-200 bg-white px-3 py-2 pl-9 text-sm placeholder-gray-400 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-200/60" />
              </div>
              <p id="note-help" className="text-xs text-gray-500">メモ（任意）</p>
            </label>
            ...
            <div className="space-y-2 pt-2" aria-live="polite">
              ...
              {error && (
                <div className="rounded-md border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">Error: {error}</div>
              )}
            </div>
          </div>
        </Card>
