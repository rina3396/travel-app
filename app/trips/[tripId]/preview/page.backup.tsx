...
export default async function TripPreviewPage({ params }: { params: { tripId: string } }) {
  ...
  // Fetch day dates for mapping if needed
  try {
          ...
          <ul className="divide-y">
            {orderedActivities.length === 0 ? (
              ...
              orderedActivities.map((a) => {
                const date = a.day_id ? dayMap[a.day_id] : undefined
                return (
                  <li key={a.id} className="flex items-start gap-3 p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        {date && (
                          <span className="inline-flex shrink-0 items-center rounded-full bg-gray-50 px-2 py-0.5 text-xs text-gray-700">
                            {date}
                          </span>
                        )}
                      </div>
