// app/trips/[tripId]/page.tsx — 旅行ダッシュボード
...
export default async function TripDashboardPage({ params }: { params: { tripId: string } }) {
  ...
  if (error || !trip) {
    return (
      <section className="p-4 space-y-4">
        <h1 className="text-2xl font-bold text-red-600">取得に失敗しました</h1>
  ...
  const start = trip.start_date ?? "開始未設定"
  const end = trip.end_date ?? "終了未設定"
  const period = `${start} - ${end}`

  return (
    <section className="space-y-8 p-4">
      <header className="space-y-1">
        <h1 className="text-2xl font-bold">旅行ダッシュボード</h1>
        <p className="text-sm text-gray-600">tripId: {tripId}</p>
      </header>
      ...
      <Card>
        <div className="flex items-center justify-between gap-4">
          ...
          <Button href={`/trips/${encodeURIComponent(tripId)}/settings`} variant="outline" size="sm">設定</Button>
        </div>
      </Card>
